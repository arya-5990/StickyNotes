...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // User will be automatically set via the auth state listener
    } catch (error) {
  ...
  const handleLogout = async () => {
    try {
      await signOutUser();
      // User will be automatically cleared via the auth state listener
    } catch (error) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="bg-white p-8 rounded-xl shadow-2xl max-w-md w-full">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{config.app.name}</h1>
          <p className="text-gray-600">{config.app.description}</p>
        </div>
